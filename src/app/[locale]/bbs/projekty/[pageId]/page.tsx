...
export const metadata: Metadata = {
  title: "Edycja projektu klienta — Panel wewnętrzny",
  ...
  try {
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 lg:px-8">
        <Link href={`/${locale}/bbs/projekty`} className="text-xs text-white/40 hover:text-white/70">← Projekty klientów</Link>
    ...
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
      <div className="mb-7">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <h1 className="text-2xl font-bold text-white">{project.name || "Projekt klienta"}</h1>
          <span className={project.active ? "rounded-full bg-emerald-400/10 px-2 py-0.5 text-[11px] text-emerald-300" : "rounded-full bg-white/5 px-2 py-0.5 text-[11px] text-white/40"}>
            {project.active ? "aktywny" : "wyłączony"}
          </span>
        </div>
        <p className="mt-1 text-sm text-white/45">{project.projectNumber || "Bez numeru projektu"}</p>
