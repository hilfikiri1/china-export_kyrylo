    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-bold text-white sm:text-3xl">{ru ? "Кейсы" : "Case'y"}</h1>
          <p className="mt-2 text-sm text-white/50">
            {ru
              ? "Управляйте публикацией, редактированием и архивом раздела Реализации."
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-navy-light/60">
        {cases.length === 0 ? (
          <div className="p-8 text-center text-sm text-white/50">
            {ru ? "В Notion пока нет кейсов." : "Brak case'ów w Notion."}
          </div>
        ) : (
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      {item.archived ? (ru ? "Архив" : "Archiwum") : statusLabel(item.status)}
                    </span>
                    {item.featured && (
                ...
                <div>
                  <p className="text-[11px] uppercase tracking-wider text-white/30">{ru ? "Дата" : "Data"}</p>
                  <p className="mt-1 text-sm text-white/65">{item.publishedAt || "—"}</p>
                </div>

                <div>
                  <p className="text-[11px] uppercase tracking-wider text-white/30">{ru ? "Категория" : "Kategoria"}</p>
