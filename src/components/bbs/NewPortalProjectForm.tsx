  ...
  if (state.status === "success" && state.pageId && state.accessUrl) {
    return (
      <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-5 sm:p-6">
        <p className="font-semibold text-white">Projekt i link klienta są gotowe</p>
        ...
        <div className="mt-5">
          ...
          <div className="flex items-start rounded-lg border border-white/10 bg-white/5 px-3 py-2">
            ...
            <CopyButton value={state.accessUrl} label="Kopiuj link klienta" />
          </div>
        </div>
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <div className="rounded-lg border border-accent-light/20 bg-accent-light/5 px-4 py-3 text-xs leading-relaxed text-white/60">
        Projekt zostanie zapisany w bazie <strong className="text-white/80">Klienci i projekty</strong> w Notion.
        System utworzy stały, bezpieczny link do panelu klienta.
      </div>

      <div className="space-y-2">
        <Label htmlFor="portal-name" className="text-white/80">
          Nazwa projektu / produktu <span className="text-accent-light">*</span>
        </Label>
        <Input id="portal-name" name="name" required maxLength={200} placeholder="np. Linia pakująca do produkcji" className={fieldClassName} />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="portal-number" className="text-white/80">Numer projektu</Label>
          <Input id="portal-number" name="projectNumber" maxLength={100} placeholder="Automatyczny, jeśli puste" className={fieldClassName} />
        </div>
        <div className="space-y-2">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="portal-company" className="text-white/80">Firma</Label>
          <Input id="portal-company" name="company" maxLength={200} placeholder="Nazwa firmy klienta" className={fieldClassName} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="portal-contact" className="text-white/80">Osoba kontaktowa</Label>
          <Input id="portal-contact" name="contactName" maxLength={200} placeholder="Imię i nazwisko" className={fieldClassName} />
        </div>
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="portal-email" className="text-white/80">E-mail klienta</Label>
          <Input id="portal-email" name="email" type="email" maxLength={320} placeholder="klient@firma.pl" className={fieldClassName} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="portal-phone" className="text-white/80">Telefon klienta</Label>
          <Input id="portal-phone" name="phone" type="tel" maxLength={80} placeholder="+48 ..." className={fieldClassName} />
        </div>
      </div>

      <div className="space-y-2">
        <Label htmlFor="portal-description" className="text-white/80">Opis / specyfikacja</Label>
        <Textarea id="portal-description" name="description" rows={4} maxLength={2000} placeholder="Krótki zakres projektu i ustalenia" className={fieldClassName} />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="portal-status" className="text-white/80">Status wewnętrzny</Label>
        ...
        <div className="space-y-2">
          <Label htmlFor="portal-stage" className="text-white/80">Etap widoczny dla klienta</Label>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="portal-next" className="text-white/80">Następny krok</Label>
          <Input id="portal-next" name="nextStep" maxLength={700} placeholder="np. Potwierdzenie próbki" className={fieldClassName} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="portal-date" className="text-white/80">Planowana data</Label>
          <Input id="portal-date" name="plannedDate" type="date" className={fieldClassName} />
        </div>
      </div>

      <div className="space-y-2">
        <Label htmlFor="portal-manager" className="text-white/80">Opiekun projektu</Label>
      ...
      <Button type="submit" disabled={pending} className="w-full bg-accent-light text-white shadow-lg shadow-accent-light/25 hover:bg-[#dbaa47]">
        {pending ? "Tworzenie projektu…" : "Utwórz projekt i link klienta"}
      </Button>
    </form>
  );
}
